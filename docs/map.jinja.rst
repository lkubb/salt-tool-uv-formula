...
We will see a quick setup to configure the ``tool_uv`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_uv`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_uv`` formulas under ``/srv/salt/tool_uv/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_uv/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_uv/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_uv/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_uv/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_uv`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_uv`` formulas under ``/srv/salt/tool_uv/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_uv/parameters/roles
...
We create a configuration for the role ``tool_uv/server`` in ``/srv/salt/tool_uv/parameters/roles/tool_uv/server.yaml``:
...
We create another configuration for the role ``tool_uv/client`` in ``/srv/salt/tool_uv/parameters/roles/tool_uv/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_uv/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_uv/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_uv/parameters/dns:domain/  '/srv/salt/tool_uv/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-uv-as-json:
      test.nop:
        - name: {{ uv | json }}
...
.. code-block:: sls
    ...
    uv-config-file-file-managed:
      file.managed:
        - name: {{ uv.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='uv-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``uv`` context variable to the jinja template which could be used like this:
